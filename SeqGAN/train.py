...
class Trainer(object):
    ...
    def __init__(self, B, T, g_E, g_H, d_E, d_H, d_dropout, g_lr=1e-3, d_lr=1e-3,
        ...
        self.top = os.getcwd()
        self.path_pos = os.path.join(self.top, 'dataset3', 'train.txt')
        self.path_neg = os.path.join(self.top, 'dataset3', 'save', 'generated_sentences.txt')
    ...
    def pre_train_generator(self, g_epochs=3, g_pre_path=None, lr=1e-3):
        if g_pre_path is None:
            self.g_pre_path = os.path.join(self.top, 'dataset3', 'save', 'generator_pre.hdf5')
        else:
    ...
    def pre_train_discriminator(self, d_epochs=1, d_pre_path=None, lr=1e-3):
        if d_pre_path is None:
            self.d_pre_path = os.path.join(self.top, 'dataset3', 'save', 'discriminator_pre.hdf5')
        else:
    ...
    def train(self, steps=10, g_steps=1, d_steps=1, d_epochs=1,
        g_weights_path='dataset3/save/generator.pkl',
        d_weights_path='dataset3/save/discriminator.hdf5',
        verbose=True,
        head=1):
